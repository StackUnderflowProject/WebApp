                        ...
                        <form id="uploadProfilePicture" onSubmit={(e) => uploadProfilePicture(e)}>
                            ...
                            (<div className='preview-wrapper'>
                                <div className='image-preview-container'>
                                    <button onClick={() => setShowUploadButton(false)} id="close-preview"><FontAwesomeIcon icon={faClose}/></button>
                    ...
                    <div className="user-details">
                        {editUserInfo ? (
                            <form className="update-user-form" onSubmit={e => updateUserInfo(e)}>
                                <button onClick={() => setEditUserInfo(false)} id="edit-user-info-button"><FontAwesomeIcon icon={faClose}/></button>
                                <h1>Vnesi nove podatke</h1>
                                <label htmlFor='username-input'>Uporabniško ime:</label>
                                <input value={userUsername} onChange={e => setUserUsername(e.target.value)} type="text" id="username-input" required/>
                                <label htmlFor='mail-input'>Email:</label>
                                <input value={userEmail} onChange={e => setUserEmail(e.target.value)} type="email" id="mail-input" required/>
                                {showPasswordLabel ? (<><label htmlFor='password-input'>Novo geslo:</label><input type="password" id="password-input" required/><label htmlFor='confirm-password-input'>Ponovi geslo:</label><input type="password" id="confirm-password-input" required/></>) : (<></>)}
                                {error !== "" ? (<p id="error-message">{error}</p>) : (<></>)}
                                <div style={{display: "flex", justifyContent: "space-between", alignItems: "center"}}>
                                    {!showPasswordLabel ? (<button id="password-toggle-button" onClick={() => setShowPasswordLabel(true)}><div style={{display: "flex", alignItems: "center", gap: "5px"}}><p>Spremeni geslo</p><FontAwesomeIcon icon={faLock}/></div></button>) : (<></>)}
                                    <button id="submit-user-update-btn" type="submit"><div style={{display: "flex", alignItems: "center", gap: "5px"}}><p>Spremeni</p><FontAwesomeIcon icon={faCheck}/></div></button>
                                </div>
                            </form>
                        ) 
                        : (
                            <>
                            {user && user._id == userId ? (<button onClick={() => setEditUserInfo(true)} id="edit-user-info-button"><FontAwesomeIcon icon={faEdit}/></button>) : <></>}

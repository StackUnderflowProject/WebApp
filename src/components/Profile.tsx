...
function Profile() {
  ...
  const getUser = async () => {
    ...
    if (response.ok) {
        console.log(data)
        if (data.image) {setProfilePicture("http://localhost:3000/images/profile_pictures/" + data.image)}
        else {setProfilePicture("../../default.png")}
  ...
  const uploadProfilePicture = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    let image: File = fileUploadElement?.files[0] ?? null;
        ...
        <div className="profile-container">
            <div className="profile-info">
                <div className="image-container">
                    <img src={profilePicture} onMouseEnter={() => setProfilePictureModifier(true)} alt="Profile" className="profile-picture" />
                    {profilePictureModifier ? (<img id="hover-picture" onClick={openFileExplorer} src="../../public/uploadProfilePicture.png" onMouseLeave={() => setProfilePictureModifier(false)}  alt="Profile" className="profile-picture" />) : (<></>)}
                </div>
                {(user && user._id == userId) ? 
                (<form id="uploadProfilePicture" onSubmit={e => uploadProfilePicture(e)}>
                    <label htmlFor="file-upload" className="custom-file-upload">
                        <input style={{ display: 'none' }} id="file-upload" type="file" name="profile_picture" accept="image/*"/>
                    </label>

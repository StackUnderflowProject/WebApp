...
export const Schedule = () => {
    ...
    return (
        <div className="mt-8 relative">
            {showScrollButton && (
                <button onClick={scrollToToday} className="fixed right-4 bottom-4 bg-gray-600 w-fit p-4">
                    Top
                </button>
            )}
            <div className="w-max absolute p-4 right-0 flex gap-2">
                <select onChange={handleSportChange} className="text-black rounded-xl p-2">
                    <option value="football">Football</option>
                    <option value="handball">Handball</option>
                </select>
                <select className="p-2 text-black rounded-xl" onChange={handleSeasonChange} value={season}>
            ...
            {Object.entries(matchesByDate)
                ...
                .map(([date, dateMatches]) => (
                    <div key={date} className="bg-gray-700 rounded-xl p-4 my-4 flex flex-col gap-4">
                        ...
                        {dateMatches.map((match) => {
                            return (
                                <div className="flex w-full">
                                    <div
                                        key={match._id}
                                        className="bg-gray-400 rounded-xl flex justify-evenly items-center p-4 w-full"
                                        id={match._id}
                                    >
                                        <div
                                        ...
                                        >
                                            <h2 className="text-black text-xl">{match.home.name}</h2>
                                            <img
                                        ...
                                        <h1 className="text-2xl p-4 bg-black rounded-xl w-1/5">
                                        ...
                                        <div
                                            ...
                                            className="w-2/5 flex flex-col gap-2 items-center justify-evenly"
                                        >
                                            <h2 className="text-black text-xl">{match.away.name}</h2>
                                            <img
                                                className="rounded-xl max-h-24 max-w-24"

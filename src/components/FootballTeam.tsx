...
export default function FootballTeam() {
    ...
    const getFootballStandings = async () => {
        try {
            const response = await fetch('http://localhost:3000/footballStanding/filterBySeason/' + new Date().getFullYear());
            const data = await response.json();
            if (response.ok) {
                setTeams(data);
            } else {
    ...
    const getFootballMatches = async () => {
        try {
          const response = await fetch('http://localhost:3000/footballMatch/filterByTeamAndSeason/' + new Date().getFullYear() + "/" + team?._id);
          const data = await response.json();
          if (response.ok) {
                                ...
                                <tr id={match.score === "" ? "toBePlayed" : undefined} key={match._id} className="match-row">
                                    ...
                                    <td>
                                        <div className="center-wrapper-small">
                                            <div onClick={() => navigate("/footballTeam/" + match.home._id)} className="home-team-small">
                                            ...
                                            <div onClick={() => navigate("/footballTeam/" + match.away._id)} className="away-team-small">

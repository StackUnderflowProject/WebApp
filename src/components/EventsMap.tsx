...
const MainMap: React.FC<MainMapProps> = ({ events, deleteEvent, followEvent, following, formatDateString }) => {
    ...
    return (
        <div className="map-M">
            <button onClick={switchTileLayer} id="map-mode-M"><strong>Prikaz</strong></button>
            <div className="map-container-M">
                <MapContainer center={[46.1512, 14.9955]} zoom={8} style={{ height: "70vh", width: "100%" }}>
                    <TileLayer
                        url={tileLayerURL}
                            ...
                            <Popup>
                                ...
                                <div className="head-M">
                                    <div className="host-info-M">
                                        <img src={(event.host.image) ? ("http://localhost:3000/images/profile_pictures/" + event.host.image) : ("/defaultProfilePicture.png")} alt={event.host.username} className="host-image-M" />
                                        <p className="host-name-M">{event.host.username}</p>
                                    </div>
                                    {(user && (user.isAdmin || user._id === event.host._id)) ? (<button onClick={() => deleteEvent(event._id)} className="delete-event-M"><FontAwesomeIcon icon={faTrash} /></button>) : (<></>)}
                                </div>
                                <div className="event-info-M">
                                    <h3 className="event-title-M">{event.name}</h3>
                                    <p className="event-description-M">{event.description}</p>
                                    <p className="event-date-M">Dogodek se prične <strong>{formatDateString(event.date)}</strong> ob <strong>{event.time}</strong></p>
                                    <p className="event-activity-M">{event.activity} <FontAwesomeIcon icon={event.activity === "nogomet" ? faSoccerBall : faPersonRunning}/></p>
                                </div>
                                <div className="follow-M">
                                    <p className="followers-display-M">{(user && following.includes(event._id) && !event.followers.includes(user._id)) ? event.followers.length + 1 : (user && !following.includes(event._id) && event.followers.includes(user._id)) ? event.followers.length - 1 : event.followers.length} <FontAwesomeIcon icon={faUser} /></p>
                                    <button onClick={e => followEvent(event, e)} className="follow-button-M">{(user && (following.includes(event._id) || event.followers.includes(user._id))) ? "Sledim ✓" : "Sledi +"}</button>
                                </div>
                        </div>
                            </Popup>
                        </Marker>
